            ...
            <li class="nav-item has-treeview menu-open">
              ...
              <ul class="nav nav-treeview ">
                ...
                <li class="nav-item has-treeview menu-open">
                  ...
                  <ul class="nav nav-treeview">
                    <li class="nav-item">
                      <a href="/caixa/index" class="nav-link active">
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Gerenciamento de Caixa </li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
                    ...
                    <form action="{{route('caixa.search')}}" method="post">
                      ...
                      <div class="form-row">
                        ...
                        <div class="form-group col-md-4">
                          ...
                          <select class="form-control select text-center" name="filter3"  id="filter3" ><br>
                            @php
                                $banco_filter=$bancos->find($filters['filter3'] ?? "");
                            @endphp
                            @if ($banco_filter==null)
                            <option  value="{{ $filters['filter3'] ?? ''}}">{{$banco_filter->name ?? 'Selecione o Banco'}}</option>
                            @else
                               <option  value="{{ $filters['filter3'] ?? ''}}">{{$banco_filter->name ?? 'Selecione a Banco'}}</option>
                            <option  value="{{''}}">{{ 'Selecione a Banco'}}</option>

                            @endif
                            @foreach($bancos as $banco)
                              @if ($banco_filter!==null)
                                @if ($banco_filter->id==$banco->id)
                      ...
                      <div class="main-cad-cadastro"> 
                        <div class="card-body table-responsive p-0" style="height: 1000px;width:100%">  
                          <table class="table table-bordered data-table table-head-fixed text-center" id="example" >
                              <thead >
                                <tr>
                                  <th scope="col" class="bg-primary">Cod.Caixa</th>
                                  <th scope="col" class="bg-primary">Nome</th>
                                  <th scope="col" class="bg-primary">Banco</th>
                                  <th scope="col" class="bg-primary">Valor Atual</th>
                                ...
                                @foreach($caixas as $caixa )
                                  <tr role="row" class="odd">
                                    @php                                             
                                      ...
                                      $banco=$caixa->find($caixa->id)->relBanco;
                                      $value=inteiro_decimal_br_ponto($caixa->value);
                                    @endphp
                                    <td>{{$caixa->id}}</td>
                                    <td>{{$caixa->name}}</td>
                                    <td>{{$banco->name}}</td>
                                    ...
                                    <td class="td-row" style="display:flex; flex-direction: row; align-items: center;justify-content: center;">
                                      <a href="{{route('edit.caixa',$caixa->id)}}">
                                        <button class="btn btn-primary" onclick="fetchContent(this)">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                      </a>
                                      <form class="form-danger" id="{{$caixa->id}}" action="{{route('destroy.caixa',$caixa->id)}}"  method="POST">
                                        @csrf
                                        @method('DELETE')
